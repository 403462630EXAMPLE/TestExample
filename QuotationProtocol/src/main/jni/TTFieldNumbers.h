#ifndef __TT_FIELD_NUMBERS_H
#define __TT_FIELD_NUMBERS_H

#define MESSAGE_MODE_RAW 0
enum FieldType{
	TTUNTYPE ,
	TTINT8 ,
	TTUINT8 ,
...
namespace MESSAGE{
	...
	const int LOGON				=	1		;
	const int LOGOUT			=	2		;
	const int SUBSCRIBE			=	3		;
	const int UNSUBSCRIBE		=	4		;
	const int MARKETDATA		=	5		;
	const int NEWS				=	6		;
	const int ALERT				=	7		;
	const int MANAGE			=	8		;
	const int SUBSCRIBE_RESULT	=	9		;
	const int TEST				=	10		;
	const int MINUTEQUOTE		=	11		;
	const int REQUESTSORT		=	12		;
	const int SNAPSHOT			=	13		;
	const int BESTQUOTE			=	14		;
	const int LATESTQUOTE		=	15		;
	const int REQUESTFILTER		=	16		;
	const int DEALQUOTE			=	17		;
	const int SECURITYLIST		=	18		;
	const int MARKETINFO			=	19		;
	const int KICKUSER		    	=	20		;
	const int DAYQUOTE		    	=	21		;
	const int TODAYMINQUOTE	=	22		;
	const int HISFIFMINQUOTE		=	23		;
	const int HISMINQUOTE		=	24		;
	const int UPDATE				=	25		;
	const int LATESTQUOTE_TPME 	=	26		;
	const int LATESTQUOTE_SGE	=	27		;
	const int LATESTQUOTE_INAU	=	28		;
	const int LATESTQUOTE_WGIN =	29		;
	const int NEWS_PUSH			=	30		;
	const int NEWS_REQUEST		=	31		;
	const int LATESTQUOTE_HJ	=	32		;
	const int QUOTE_HJ_REQ		=	33		;
	const int PRICE_TABLE_REQ		=	34		;
	const int ORDER_LIST_REQ		=	35		;
	const int SECURITY_NEWS_REQ		=	36		;
	const int LATESTQUOTE_GZ = 37;
	const int DiLei_PUSH 			=	38		;
	const int DiLei_REQUEST 		=	39		;
	const int RELAY_NEWS             =   	40;
	const int LATESTQUOTE_WH =	41;
	const int LATESTQUOTE_GJZS	=	42;
	const int LATESTQUOTE_SHFE	= 	43		;//上海
	const int LATESTQUOTE_DCE	= 	44		;//大连
	const int LATESTQUOTE_CZCE	= 	45		;//郑州
	const int BSSIGNAL_PUSH 		=	46		;//买卖点信号推送
	const int BSSIGNAL_REQUEST 	=	47		;//买卖点信号请求数据
	const int NEWSINFO_PUSH 	=	48		;//新闻推送
	const int NEWSINFO_REQUEST 	=	49		;//新闻请求
	const int CHANGEHQDEAL = 50;				//可控行情处理类型
	const int DAYBSSIGNAL_REQUEST 	=	51		;//买卖点信号请求数据
	const int FINANCE_REQUEST 	=	52		;////财经日历请请求数据
	const int TRADECALENDAR_REQUEST = 53 ;	//财经日历请求
	const int TRADECALENDAR_PUSH = 54 ;		//财经日历推送
	const int MODIFYKDATA		=    55		;//通知入库修改K线数据
	const int NOTIFYHQKDATA		=    56		;//通知行情服务器重新下载K线数据
	const int HQUPDATE			=	57		;//通知行情服务器更新快照或者当天分钟线
	const int DATAVERSION_REQ	=	58;		//版本信息请求
	const int CJRLUPDATE			= 	60		;//财经日历数据更新
	const int KXUPDATE			= 	59		;//快讯数据更新
	const int CJRLUPDATENOTIFY = 61;
	const int LATESTQUOTE_LME	= 	70;	
	const int CREATE_QUOTE	= 	80;	
	const int LATESTQUOTE_BPEX	= 	81;//北京石油交易所行情
	const int FASTNEWS_REQUEST = 100 ;     	//快讯请求
	const int FASTNEWS_PUSH = 101 ;			//快讯推送
	const int SENDENNEWS_REQUEST = 102 ;	//森德定制新闻
	const int SENDENNEWS_PUSH = 103 ;		//森德定制新闻
	const int OPENINTEREST_REQUEST = 104 ;//持仓
	const int OPENINTEREST_PUSH = 105 ;//持仓
	const int BUYSELLPOSTIONRATIO_REQUEST = 106;//多空持仓比
	const int BUYSELLPOSTIONRATIO_PUSH = 107;//多空持仓比
	const int BUYSELLPOSTIONTREND_REQUEST = 108;//多空趋势比
	const int BUYSELLPOSTIONTREND_PUSH = 109;//多空趋势比
	const int SENDEDILEI_PUSH = 110;//森德地雷
	const int SENDERELATION_PUSH = 111;//森德订阅列表	
	const int AD_PUSH = 112;//广告推送	
	const int AD_REQUEST = 113;//广告请求   ^M
	const int LSP_POSITION_PUSH = 114;//LSP持仓数据推送   ^M
	const int LSP_POSITION_REQUEST = 115;//LSP持仓数据请求   ^M
	const int LSP_FUND_PUSH = 116;//LSP资金数据推送   ^M
        const int LSP_FUND_REQUEST = 117;//LSP资金数据请求   ^M
	const int CALC_SIGNAL_TAIJI = 150;//太极图信号
	const int MESSAGE_MAX		=	255		;
	const int INVALID_MESSAGE	=  -1		;
}

/*namespace FIELD
{
	const int	EndDateTime		= 12011 ;
	const int	Market			= 11022 ;
	const int SecurityID = 48;
	const int	AbbrName		= 12001 ;
	const int	DateTime		= 11001 ;
	const int LatestPx		=	10129 ;
	const int BidPx = 132;
	const int	AskPx			= 11038 ;
	const int OpenPx		=	10130 ;
	const int HighPx = 332;
	const int LowPx = 333;
	const int PrevClosePx = 140;
	const int	TotalTradeVolume= 11015 ;
	const int	TotalTradeAmount= 11016 ;
	const int	PreOpenPosition	=	10191	;
	const int  BidPx1			=	10175  ;
	const int	BidVolume1		= 11041 ;
	const int  BidPx2			=	10173  ;
	const int	BidVolume2		= 11051 ;
	const int  BidPx3			=	10171  ;
	const int	BidVolume3		= 11061 ;
	const int  BidPx4			=	10169  ;
	const int	BidVolume4		= 11071 ;
	const int  BidPx5			=	10167  ;
	const int	BidVolume5		= 11081 ;
	const int	AskPx1			= 11048 ;
	const int	AskVolume1		= 11045 ;
	const int	AskPx2			= 11058 ;
	const int	AskVolume2		= 11055 ;
	const int	AskPx3			= 11068 ;
	const int	AskVolume3		= 11065 ;
	const int	AskPx4			= 11078 ;
	const int	AskVolume4		= 11075 ;
	const int	AskPx5			= 11088 ;
	const int	AskVolume5		= 11085 ;
	const int	PreSettlementPx	=	10190	;

	const int MSGSEQ			= 60000 ;
	const int StringField		= 60001 ;
	const int BoolField			= 60002 ;
	const int DateTimeField		= 60003 ;
	const int I8Field			= 60004 ;
	const int U8Field			= 60005 ;
	const int I16Field			= 60006 ;
	const int U16Field			= 60007 ;
	const int I32Field			= 60008 ;
	const int U32Field			= 60009 ;
	const int I64Field			= 60010 ;
	const int U64Field			= 60011 ;
	const int F32Field			= 60012 ;
	const int F64Field			= 60013 ;
	}*/
  namespace FIELD
  {
	...
	namespace FIELD{
		const int SecurityCode	=	10100 ;
		const int QuoteSeqID	=	10101 ;
		const int NoInstrument	=	8509  ;
		const int NoSubject		=	10102 ;
		const int Summary		=	10103 ;
		const int Token			=	10104 ;
		const int AlertType		=	10105 ;
		const int SubjectName	=	10106 ;
		const int SubjectCode	=	10107 ;

		const int IsIndexLine	=	10108 ;
		const int Origin		=	10109 ;
		const int FilterRuleID	=	10110 ;
		const int ChannelType	=	10111 ;
		const int Board			=	10112 ;

		const int ListMarketID	=	10113 ;
		const int MarketTradingTarget = 10114 ;
		const int BidTime		=	10116 ;
		const int BidContraRate	=	10117 ;
		const int BidSettlmntTyp=	10118 ;
		const int BidFlegStlmntMthd=10119 ;
		const int OfferContraRate=	10120 ;
		const int OfferSettlmntTyp= 10121 ;
		const int OfferFlegStlmntMthd=10122 ;
		const int MDEntryDateTime=  10123 ;
		const int Chge			=	10124 ;
		const int ChgeRate		=	10125 ;
		const int TotalTradedAmnt=  10126 ;
		const int TradedAmnt	=	10127 ;
		const int TradeQuantity=	10128 ;
		const int LatestPx		=	10129 ;
		const int OpenPx		=	10130 ;
		const int ClosePx		=	10131 ;
		const int HighstPx		=	10132 ;
		const int LwstPx		=	10133 ;
		const int WghtdPx		=	10134 ;
		const int PreClosePx	=	10135 ;
		const int PreWghtdPx	=	10136 ;
		const int LatestContraRate=	10137 ;
		const int OpenContraRate=	10138 ;
		const int CloseContraRate=	10139 ;
		const int HighstContraRate=	10140 ;
		const int LwstContraRate=	10141 ;
		const int WghtdContraRate=	10142 ;
		const int PreCloseContraRate=10143;
		const int PreWghtdContraRate=10144 ;
		const int EntySideNo	=	10145 ;
		const int Command		=	10146 ;
		const int ResultInfo	=	10147 ;
		const int ResultPageNo	=	10148 ;
		const int ResultPageEnd =	10149 ;

		const int  TotalValueTraded	=	10150   ;
		const int  NumberOfTrades	=	10151  ;
		const int  PERatio1			=	10152  ;
		const int  PERatio2			=	10153  ;
		const int  Change1			=	10154  ;
		const int  Change2			=	10155  ;
		const int  OpenPosition		=	10156  ;
		const int  OfferPx5			=	10157  ;
		const int  OfferSize5		=	10158  ;
		const int  OfferPx4			=	10159  ;
		const int  OfferSize4		=	10160  ;
		const int  OfferPx3			=	10161  ;
		const int  OfferSize3		=	10162  ;
		const int  OfferPx2			=	10163  ;
		const int  OfferSize2		=	10164  ;
		const int  OfferPx1			=	10165  ;
		const int  OfferSize1		=	10166  ;
		const int  BidPx5			=	10167  ;
		const int  BidSize5			=	10168  ;
		const int  BidPx4			=	10169  ;
		const int  BidSize4			=	10170  ;
		const int  BidPx3			=	10171  ;
		const int  BidSize3			=	10172  ;
		const int  BidPx2			=	10173  ;
		const int  BidSize2			=	10174  ;
		const int  BidPx1			=	10175  ;
		const int  BidSize1			=	10176  ;
		const int  MessageType		=	10177  ;
		const int  TradedVolume		=	10178	;
		const int  TradedValue		=	10179	;
		const int	BidQuoteSide	=	10180	;
		const int	BidDirtyPx		=	10181	;
		const int	OfferQuoteSide	=	10182	;
		const int	OfferQuoteTime	=	10183	;
		const int	OfferDirtyPx	=	10184	;
		const int	AccruedInterest	=	10185	;
		const int	UpdateTime		=	10186	;
		const int	UpdateMillisec	=	10187	;
		const int	SettlementGroupID=	10188	;
		const int	SettlementID	=	10189	;
		const int	PreSettlementPx	=	10190	;
		const int	PreOpenPosition	=	10191	;
		const int	SettlementPx	=	10192	;
		const int	UpperLimitPx	=	10193	;
		const int	LowerLimitPx	=	10194	;
		const int	PreDelta		=	10195	;
		const int	CurrDelta		=	10196	;

		const int	RequestID		=	10200	;
		const int	StartTime		=	10201	;
		const int	MinuteSpan		=	10202	;
		const int	TotalMessage	=	10203	;
		const int	Sequence		=	10204	;
		//const int	AvgPx			=	10205	;
		const int	RequestResult	=	10206	;
		const int	GroupExpr		=	10207	;
		const int	FieldName		=	10208	;
		const int	SortBegin		=	10209	;
		const int	SortSize		=	10210	;
		const int	SortRespType	=	10211	;
		const int	ListCount		=	10212	;
		const int	GroupID			=	10213	;
		const int	AscOrder		=	10214	;

		const int	BidContraRate1 = 10215	;
		const int	BidContraRate2 = 10216	;
		const int	BidContraRate3 = 10217	;
		const int	BidContraRate4 = 10218	;
		const int	BidContraRate5 = 10219	;

		const int	OfferContraRate1 = 10220	;
		const int	OfferContraRate2 = 10221	;
		const int	OfferContraRate3 = 10222	;
		const int	OfferContraRate4 = 10223	;
		const int	OfferContraRate5 = 10224	;
		const int	ID					= 10225 ;
		const int	OpenClosedSign		= 10226 ;
		const int	Keywords		=	10227 ;

		const int	TTPreClosePx	=	10228	;
		const int	TTPreCloseContraRate=	10229	;

		const int	BidMinQty1 = 10230	;
		const int	BidMinQty2 = 10231	;
		const int	BidMinQty3 = 10232	;
		const int	BidMinQty4 = 10233	;
		const int	BidMinQty5 = 10234	;

		const int	OfferMinQty1 = 10235	;
		const int	OfferMinQty2 = 10236	;
		const int	OfferMinQty3 = 10237	;
		const int	OfferMinQty4 = 10238	;
		const int	OfferMinQty5 = 10239	;

		const int	BidQuoteSide1 = 10240	;
		const int	BidQuoteSide2 = 10241	;
		const int	BidQuoteSide3 = 10242	;
		const int	BidQuoteSide4 = 10243	;
		const int	BidQuoteSide5 = 10244	;

		const int	OfferQuoteSide1 = 10245	;
		const int	OfferQuoteSide2 = 10246	;
		const int	OfferQuoteSide3 = 10247	;
		const int	OfferQuoteSide4 = 10248	;
		const int	OfferQuoteSide5 = 10249	;

		const int	BidTime1		= 10250	;
		const int	BidTime2		= 10251	;
		const int	BidTime3		= 10252	;
		const int	BidTime4		= 10253	;
		const int	BidTime5		= 10254	;

		const int	OfferQuoteTime1 = 10255	;
		const int	OfferQuoteTime2 = 10256	;
		const int	OfferQuoteTime3 = 10257	;
		const int	OfferQuoteTime4 = 10258	;
		const int	OfferQuoteTime5 = 10259	;

		const int	FxType			= 10260 ;
		const int	TradeCode		= 10261 ;
		const int	Millisec		= 10262 ;
		const int	PreAvrgPx		= 10263 ;
		const int	LatestYield		= 10264 ;
		const int  ChangeBP			= 10265	;

		const int	DateTime		= 11001 ;
		const int	CloseYield		= 11002 ;
		const int	OpenYield		= 11003 ;
		const int	HighYield		= 11004 ;
		const int	LowYield		= 11005 ;
		const int	WghtdYield		= 11006 ;
		const int	PrevCloseYield	= 11007 ;
		const int	PrevWghtdYield	= 11008 ;

		const int	PrevWghtdPx		= 11009 ;

		const int	Change			= 11010 ;
		const int	ChangeRate		= 11011 ;
		const int	YieldChange		= 11012 ;

		const int	TradeAmount		= 11014 ;
		const int	TotalTradeVolume= 11015 ;
		const int	TotalTradeAmount= 11016 ;
		const int	TurnOverRate	= 11017 ;

   		const int	MacaulayDuration= 11018 ;
		const int	ModifiedDuration= 11019 ;
		const int	Convexity		= 11020 ;
		const int	DV01			= 11021 ;
		const int	Market			= 11022 ;

		//const int	BidYield		= 11030 ;
		const int	BidVolume		= 11031 ;
		const int	BidMarketMaker	= 11032 ;
		const int	BidSettlType	= 11033 ;
		const int	AskYield		= 11034 ;
		const int	AskVolume		= 11035 ;
		const int	AskMarketMaker	= 11036 ;
		const int	AskSettlType	= 11037 ;
		const int	AskPx			= 11038 ;
		const int	AskTime			= 11039 ;

		const int	BidYield1		= 11040 ;
		const int	BidVolume1		= 11041 ;
		const int	BidMarketMaker1	= 11042 ;
		const int	BidSettlType1	= 11043 ;
		const int	AskYield1		= 11044 ;
		const int	AskVolume1		= 11045 ;
		const int	AskMarketMaker1	= 11046 ;
		const int	AskSettlType1	= 11047 ;
		const int	AskPx1			= 11048 ;
		const int	AskTime1		= 11049 ;

		const int	BidYield2		= 11050 ;
		const int	BidVolume2		= 11051 ;
		const int	BidMarketMaker2	= 11052 ;
		const int	BidSettlType2	= 11053 ;
		const int	AskYield2		= 11054 ;
		const int	AskVolume2		= 11055 ;
		const int	AskMarketMaker2	= 11056 ;
		const int	AskSettlType2	= 11057 ;
		const int	AskPx2			= 11058 ;
		const int	AskTime2		= 11059 ;

		const int	BidYield3		= 11060 ;
		const int	BidVolume3		= 11061 ;
		const int	BidMarketMaker3	= 11062 ;
		const int	BidSettlType3	= 11063 ;
		const int	AskYield3		= 11064 ;
		const int	AskVolume3		= 11065 ;
		const int	AskMarketMaker3	= 11066 ;
		const int	AskSettlType3	= 11067 ;
		const int	AskPx3			= 11068 ;
		const int	AskTime3		= 11069 ;

		const int	BidYield4		= 11070 ;
		const int	BidVolume4		= 11071 ;
		const int	BidMarketMaker4	= 11072 ;
		const int	BidSettlType4	= 11073 ;
		const int	AskYield4		= 11074 ;
		const int	AskVolume4		= 11075 ;
		const int	AskMarketMaker4	= 11076 ;
		const int	AskSettlType4	= 11077 ;
		const int	AskPx4			= 11078 ;
		const int	AskTime4		= 11079 ;

		const int	BidYield5		= 11080 ;
		const int	BidVolume5		= 11081 ;
		const int	BidMarketMaker5	= 11082 ;
		const int	BidSettlType5	= 11083 ;
		const int	AskYield5		= 11084 ;
		const int	AskVolume5		= 11085 ;
		const int	AskMarketMaker5	= 11086 ;
		const int	AskSettlType5	= 11087 ;
		const int	AskPx5			= 11088 ;
		const int	AskTime5		= 11089 ;

		const int	QuoteSpread		= 11100 ;
		const int	NumOfQuote		= 11101 ;
		const int	BidTotalVolume	= 11102 ;
		const int	AskTotalVolume	= 11103 ;
		const int	NumOfTrades		= 11104 ;
		const int	PERatio			= 11105 ;
		const int	Snapshot		= 11106 ;

		const int	RequestResultCode=	11107	;


		const int	FilterExpr		= 11200 ;
		const int	Begin			= 11201 ;
		const int	Size			= 11202 ;

		const int	YearsToMaturity = 12000 ;
		const int	AbbrName		= 12001 ;
		const int	IssueVolume		= 12002 ;
		const int	BondCategory	= 12003 ;
		const int	BondCreditRating= 12004 ;
		const int	BondCreditRatingAgency	= 12005 ;
		const int	CorporateCreditRating	= 12006 ;
		const int	CorporateCreditRatingAgency		= 12007 ;
		const int	CDCYieldInTheory	= 12008 ;
		const int	CDCPxInTheory	= 12009 ;
		const int	BeginDateTime	= 12010 ;
		const int	EndDateTime		= 12011 ;
		const int	TTPrevClosePx	= 12012 ;
		const int	TTPrevCloseYield= 12013 ;
		const int	PositionChange	 = 12014 ;
		const int	TotalPositionChange= 12015 ;
		const int	OffSet	=	12016 ;
		const int	ErrMsg	=	12017 ;
		const int InternalCode = 12018;

		//保留字段
		//string
		const int	ReserveString_1		=	12019	;
		const int	ReserveString_2		=	12020	;
		const int	ReserveString_3		=	12021	;
		const int	ReserveString_4		=	12022	;
		const int	ReserveString_5		=	12023	;
		const int	ReserveString_6		=	12024	;
		const int	ReserveString_7		=	12025	;
		const int	ReserveString_8		=	12026	;
		const int	ReserveString_9		=	12027	;
		const int	ReserveString_10		=	12028	;

		//int32

		const int	ReserveInt32_1		=	12029	;
		const int	ReserveInt32_2		=	12030	;
		const int	ReserveInt32_3		=	12031	;
		const int	ReserveInt32_4		=	12032	;
		const int	ReserveInt32_5		=	12033	;
		const int	ReserveInt32_6		=	12034	;
		const int	ReserveInt32_7		=	12035	;
		const int	ReserveInt32_8		=	12036	;
		const int	ReserveInt32_9		=	12037	;
		const int	ReserveInt32_10	=	12038	;

		//int 64

		const int	ReserveInt64_1		=	12039	;
		const int	ReserveInt64_2		=	12040	;
		const int	ReserveInt64_3		=	12041	;
		const int	ReserveInt64_4		=	12042	;
		const int	ReserveInt64_5		=	12043	;
		const int	ReserveInt64_6		=	12044	;
		const int	ReserveInt64_7		=	12045	;
		const int	ReserveInt64_8		=	12046	;
		const int	ReserveInt64_9		=	12047	;
		const int	ReserveInt64_10	=	12048	;

		//double
		const int	ReserveDouble_1		=	12049	;
		const int	ReserveDouble_2		=	12050	;
		const int	ReserveDouble_3		=	12051	;
		const int	ReserveDouble_4		=	12052	;
		const int	ReserveDouble_5		=	12053	;
		const int	ReserveDouble_6		=	12054	;
		const int	ReserveDouble_7		=	12055	;
		const int	ReserveDouble_8		=	12056	;
		const int	ReserveDouble_9		=	12057	;
		const int	ReserveDouble_10	=	12058	;
	...
	namespace FIELD{
		...
		const int F32Field			= 60012 ;
		const int F64Field			= 60013 ;
		const int DATA				= 60014 ;
		const int DEBUGINFO			= 60015 ;
		const int CLIENT_TYPE		= 60016;
		const int CLIENT_VERSION    = 60017;
		const int Data_Sequence    = 60018;
	}

	namespace FIELD{
		const int FIELD_MAX			= 65535 ;
	} ;
#endif /** __TT_FIELD_NUMBERS_H*/
